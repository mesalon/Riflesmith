...
public class CFXEasyEditor : EditorWindow
{
	...
	private class ParticleSystemModule
	{
		...
		public void CheckIfEnabledInSource(SerializedObject serializedSource)
		{
			...
			SerializedProperty sp = serializedSource.FindProperty(this.serializedPropertyPath);
			if(sp != null)
			{
				SerializedProperty enabled = sp.FindPropertyRelative("enabled");
				if(enabled != null)
				{
	...
	void OnGUI()
	{
		...
		Rect rect = EditorGUILayout.GetControlRect(false, EditorGUIUtility.singleLineHeight, EditorStyles.label);
...
//		GUILayout.Space(6);
	...
	void RefreshCurrentlyEnabledModules()
	{
		...
		{
			SerializedObject so = new SerializedObject(sourceObject);
			for(int i = 0; i < modulesToCopy.Length; i++)
			{
...
#if UNITY_5_5_OR_NEWER
			ParticleSystem.MainModule dmain = dest.main;
	...
	private void ScaleParticleValues(ParticleSystem ps, GameObject parent)
	{
		...
		foreach(string path in PropertiesToScale)
		{
			SerializedProperty prop = psSerial.FindProperty(path);
			if(prop != null)
			{
