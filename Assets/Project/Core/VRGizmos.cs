...
public static class VRGizmos {
	...
	static Mesh CubeMesh => cubeMesh ??= CreateCubeMesh();
	static Mesh SphereMesh => sphereMesh ??= CreateSphereMesh();
	static Material Material => material ??= CreateMaterial();

	static Material CreateMaterial() {
		// URP unlit shader - works in builds
	...
	public static void Line(Vector3 start, Vector3 end, Color color, float thickness = 0.005f) {
	...
	public static void Ray(Vector3 origin, Vector3 direction, Color color, float thickness = 0.005f) {
		Line(origin, origin + direction, color, thickness);
	}
	...
	// Axis helper
	public static void Axis(Vector3 position, float size = 0.1f, float thickness = 0.003f) {
		Ray(position, Vector3.right * size, Color.red, thickness);
		Ray(position, Vector3.up * size, Color.green, thickness);
		Ray(position, Vector3.forward * size, Color.blue, thickness);
	}

	public static void Axis(Vector3 position, Quaternion rotation, float size = 0.1f, float thickness = 0.003f) {
		Ray(position, rotation * Vector3.right * size, Color.red, thickness);
		Ray(position, rotation * Vector3.up * size, Color.green, thickness);
		Ray(position, rotation * Vector3.forward * size, Color.blue, thickness);
	}
}
