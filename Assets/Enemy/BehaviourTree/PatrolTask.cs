...
public class PatrolTask : Node {
    ...
    public override NodeState Evaluate(out Node active) {
        active = this;
        if (ctx.enemyType != EnemyType.Patrol) { return NodeState.Failure; }
        ctx.locomotion.MoveTo(patrolPath[currentPoint]);
        if (ctx.locomotion.didArrive) { currentPoint++; }
